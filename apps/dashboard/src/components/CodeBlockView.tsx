...
const mdPreviewComponents: Components = {
  ...
  p: ({ children, ...props }) => <p className="my-0.5 text-zinc-200" {...props}>{children}</p>,
  ul: ({ children, ...props }) => <ul className="list-disc ml-4 my-1 text-zinc-200" {...props}>{children}</ul>,
  ol: ({ children, ...props }) => <ol className="list-decimal ml-4 my-1 text-zinc-200" {...props}>{children}</ol>,
  blockquote: ({ children, ...props }) => (
    <blockquote className="border-l-2 border-orange-500/50 pl-3 my-1 text-zinc-400 italic" {...props}>{children}</blockquote>
  ),
  code: ({ children, ...props }) => (
    <code className="rounded bg-zinc-700/80 px-1.5 py-0.5 text-xs font-mono text-orange-300" {...props}>{children}</code>
  ),
  ...
  hr: (props) => <hr className="border-zinc-700 my-2" {...props} />,
};
  ...
  return (
    <NodeViewWrapper className="code-block-wrapper my-2">
      <div className="rounded-lg border border-zinc-700 bg-zinc-900 overflow-hidden">
        ...
        <div className="flex items-center justify-between px-3 py-1.5 border-b border-zinc-700 bg-zinc-800/60">
          <select
            ...
            className="bg-zinc-700 text-zinc-300 text-xs rounded px-2 py-0.5 border border-zinc-600 outline-none cursor-pointer hover:bg-zinc-600 transition-colors"
          >
            {LANGUAGES.map((lang) => (
          ...
          {isMarkdown && (
            <button
              ...
              className="text-xs text-zinc-400 hover:text-zinc-200 cursor-pointer px-2 py-0.5 rounded hover:bg-zinc-700 transition-colors"
            >
